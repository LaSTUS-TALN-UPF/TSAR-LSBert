...
def convert_sentence_to_token(sentence, seq_length, tokenizer,toktok):
    ...
    if "»" in sentence:
        ...
        nltk_sent = toktok.tokenize(sentence.lower())
    ...
    for i,word in enumerate(nltk_sent):
        ...
        if tokenized_text[token_index]==word or tokenized_text[token_index]=="[UNK]" or tokenized_text[token_index]=="&" or len_token>=len(word):
...
def getWordCountTXT(word_count_path):
    word2count = {}
    with open(word_count_path, "r", encoding='utf-8') as reader:
        while True:
            line = reader.readline()
            if not line:
                break
            word,freq = line.strip().split('\t',1)
            word2count[word] = round(float(freq),3)
...
def main():
    ...
    word_count = getWordCountTXT(word_count_path)
    ...
    if args.local_rank == -1 or torch.distributed.get_rank() == 0:
        ...
        for i in range(eval_size):
            ...
            tokens, words, position = convert_sentence_to_token(eval_examples[i], args.max_seq_length, tokenizer,toktok)
            ...
            #PPDB not currently used in Spanish
            ...
            cgBERT,cgScores = BERT_candidate_generation(mask_words[i], pre_tokens, predicted_top[0].cpu().numpy(), ps, args.num_selections_SG)
            ...
            print("sentence:")
            print(i)
            print(str(i)+" RESULTS_cgBERT: "+ str(cgBERT))
